...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh;">
  ...
  async sendNotificationEmail(email: string, title: string, message: string, fullName?: string) {
    const smtpUser = this.configService.get('SMTP_USER');
  ...
  async sendAchievementEmail(email: string, fullName: string, achievementName: string, achievementIcon: string, xpReward: number) {
    const smtpUser = this.configService.get('SMTP_USER');
    ...
    const content = `
      ...
      <!-- CTA Button -->
      <div style="text-align: center;">
        <a href="https://bilimdon-ai.uz/achievements" style="display: inline-block; padding: 16px 40px; background: linear-gradient(135deg, #f59e0b 0%, #eab308 100%); color: #ffffff; text-decoration: none; border-radius: 14px; font-size: 16px; font-weight: 700; box-shadow: 0 10px 25px -5px rgba(245, 158, 11, 0.5);">
          ?? Barcha yutuqlarim
        </a>
      </div>
    `;
  ...
  async sendInviteEmail(email: string) {
    ...
    const content = `
      ...
      <div style="text-align: center; margin-bottom: 25px;">
        <a href="https://bilimdon-ai.uz/auth/register" style="display: inline-block; padding: 18px 50px; background: linear-gradient(135deg, #06b6d4 0%, #0891b2 100%); color: #ffffff; text-decoration: none; border-radius: 16px; font-size: 18px; font-weight: 700; box-shadow: 0 12px 30px -5px rgba(6, 182, 212, 0.5); letter-spacing: 0.5px;">
  ...
  async sendLoginAttemptWarning(email: string, fullName: string) {
    ...
    const content = `
      ...
      <!-- CTA Button -->
      <div style="text-align: center; margin-bottom: 25px;">
        <a href="https://bilimdon-ai.uz/auth/login" style="display: inline-block; padding: 18px 50px; background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: #ffffff; text-decoration: none; border-radius: 16px; font-size: 18px; font-weight: 700; box-shadow: 0 12px 30px -5px rgba(245, 158, 11, 0.5); letter-spacing: 0.5px;">
          ?? Profilga kirish
        </a>
      </div>
  ...
  async sendAdminMessage(
    ...
    const baseUrl = this.configService.get('BACKEND_URL') || 'https://api.bilimdon-ai.uz';
    ...
    const content = `
      ...
      <div style="text-align: center; margin-top: 25px;">
        <a href="https://bilimdon-ai.uz" style="display: inline-block; padding: 14px 35px; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: #ffffff; text-decoration: none; border-radius: 12px; font-size: 15px; font-weight: 600;">
          &#127891; Bilimdon'ga o'tish
        </a>
      </div>
    `;
