...
import { Role, NotificationType, Difficulty } from '@prisma/client';
...
export class AdminService {
  ...
  async getExtendedDashboard() {
    ...
    const lowQuestionCategories = await Promise.all(
      categoriesWithQuestions.map(async (c) => {
        ...
        const unusedCount = c.questions.filter(q => !usedSet.has(q.id)).length;
  ...
  async createCategory(data: {
    ...
    this.notificationsService.notifyNewCategory({
      id: category.id,
      name: category.name,
      slug: category.slug,
      icon: category.icon,
  ...
  async importQuestionsFromText(categoryId: string, text: string) {
    ...
    if (newQuestions.length === 0) {
      return { imported: 0, skipped: questions.length, message: 'Barcha savollar allaqachon mavjud' };
    }
  ...
  async createCategoryWithQuestions(data: {
    ...
    if (questions.length < 300) {
      throw new BadRequestException(`Kamida 300 ta savol kerak. Hozir: ${questions.length} ta`);
    }

    // Check difficulty distribution (need all 3 levels)
    const difficulties = new Set(questions.map(q => q.difficulty));
    if (difficulties.size < 3) {
      throw new BadRequestException('Savollar 3 ta darajada (EASY, MEDIUM, HARD) bo\'lishi kerak');
    }
  ...
  private parseQuestionsText(text: string): Array<{
    ...
    } else if (titleLower.includes("o'rta") || titleLower.includes('orta') || titleLower.includes('medium')) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentQuestion && !line.startsWith('A)') && !line.startsWith('B)') && !line.startsWith('C)') && !line.startsWith('D)') && !line.toLowerCase().startsWith('javob')) {
  ...
  async sendMultiChannelMessage(params: {
    ...
    const { adminId, title, message, imageUrl, videoUrl, targetType, targetIds, channels, filter } = params;
    ...
    // Get target users
    let users: Array<{ id: string; email: string | null; telegramId: string | null; fullName: string }> = [];
    ...
    if (targetType === 'selected' && users.length > 15) {
      throw new BadRequestException('Bir vaqtda 15 tadan ko\'p foydalanuvchiga xabar yuborib bo\'lmaydi');
    }
    ...
    for (const user of users) {
      ...
      if (channels.includes('notification')) {
        try {
          ...
          console.log('[sendMultiChannelMessage] Creating notification with data:', notificationData);
      ...
      if (channels.includes('email') && user.email) {
        try {
          console.log('[Admin] Sending email with imageUrl:', imageUrl, 'videoUrl:', videoUrl);
      ...
      if (channels.includes('telegram') && user.telegramId) {
        try {
          ...
          await this.telegramService.sendMessage(parseInt(user.telegramId), telegramMessage);
          telegramSent++;
        } catch (error) {
