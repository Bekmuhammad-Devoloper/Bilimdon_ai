...
export class TestsService {
  ...
  async startTest(userId: string, dto: StartTestDto) {
    ...
    if (questions.length === 0) {
      throw new BadRequestException('Bu kategoriyada yetarli savol mavjud emas');
    }
  ...
  async submitTest(userId: string, testAttemptId: string, dto: SubmitTestDto) {
    ...
    if (testAttempt.categoryId) {
      await this.updateCategoryStats(userId, testAttempt.categoryId, correctAnswers, testAttempt.totalQuestions, totalXP, score);
    }
  ...
  private async updateCategoryStats(
    ...
    if (existing) {
      ...
      const newAverageScore = (existing.averageScore * existing.totalTests + score) / newTotalTests;
