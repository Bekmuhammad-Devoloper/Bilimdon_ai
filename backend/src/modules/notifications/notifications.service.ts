...
export class NotificationsService {
  ...
  async notifyNewCategory(category: {
    ...
    const webappUrl = this.configService.get<string>('WEBAPP_URL') || 'https://bilimdon.uz';
    ...
    this.logger.log(`Notifications sent - Telegram: ${telegramSent}, Email: ${emailSent}, WebSocket: ${websocketSent}, Errors: ${errors}`);
   ...
   */
  private async sendTelegramMessage(chatId: string | number, text: string, options?: {
   ...
   */
  private async sendTelegramPhoto(chatId: string | number, photoUrl: string, caption: string, options?: {
    parse_mode?: 'HTML' | 'Markdown' | 'MarkdownV2';
    reply_markup?: any;
  }) {
    if (!this.botToken) {
  ...
  private async sendTelegramPhotoBuffer(
    ...
    formData.append('photo', photoBuffer, { filename, contentType: 'image/png' });
