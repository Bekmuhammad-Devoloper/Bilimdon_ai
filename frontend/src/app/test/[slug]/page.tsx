  ...
  const handleAnswer = useCallback((optionIndex: number) => {
    ...
    setAnswers(prev => {
      ...
      if (existing !== -1) {
        ...
        newAnswers[existing] = { questionId: currentQuestion.id, selectedAnswer: optionIndex };
        return newAnswers;
      }
      return [...prev, { questionId: currentQuestion.id, selectedAnswer: optionIndex }];
    });
  }, [currentQuestion]);
  ...
  const handleSubmit = async () => {
    ...
    if (answers.length < questions.length) {
      const confirm = window.confirm(
        `Siz hali ${questions.length - answers.length} ta savolga javob bermadingiz. Yuborishni xohlaysizmi?`
      );
      if (!confirm) return;
    }
