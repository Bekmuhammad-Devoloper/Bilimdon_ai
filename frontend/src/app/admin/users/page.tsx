...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API}/admin/users?page=${page}&limit=20&search=${encodeURIComponent(searchQuery)}`, {
  ...
  const getRoleBadgeClass = (role: string) => {
    switch (role) {
      ...
      case 'MODERATOR': return 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400';
  ...
  return (
    <div className="space-y-6">
              ...
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">
                  <input
                    ...
                    onChange={() => selectedUsers.length > 0 ? setSelectedUsers([]) : setSelectedUsers(users.slice(0, 15).map(u => u.id))}
                    className="w-4 h-4 text-blue-600 rounded"
                    title="Hammasini tanlash"
                  />
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Foydalanuvchi</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Kontakt</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Rol</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">XP</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Testlar</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Amallar</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {users.map(user => (
                <tr key={user.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${selectedUsers.includes(user.id) ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}>
                  ...
                  <td className="px-4 py-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1 truncate max-w-[200px]">
                        <span>📧</span> {user.email || <span className="text-gray-400 italic">yo'q</span>}
                      </p>
                        ...
                        <div className="text-sm">
                          ...
                          {user.telegramPhone && (
                            <p className="text-gray-500 dark:text-gray-400 text-xs pl-5 flex items-center gap-1">
                              <span className="text-red-500">📞</span> {formatPhoneNumber(user.telegramPhone)}
                            </p>
                          )}
                        </div>
                      ) : null}
                  ...
                  <td className="px-4 py-4">
                    <select
                      ...
                      className={`px-2 py-1 rounded-lg text-sm font-medium ${getRoleBadgeClass(user.role)} border-0 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50`}
                      title={isReadOnly ? "Faqat ko'rish rejimi" : "Rolni o'zgartirish"}
                    >
                  ...
                  <td className="px-4 py-4">
                    <span className="font-medium text-gray-900 dark:text-white">{user.totalXP || 0}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-gray-600 dark:text-gray-400">{user.completedTests || 0}</span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex gap-2">
                      <button
      ...
      <div className="md:hidden space-y-3">
        {users.map(user => (
          <div key={user.id} className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow ${selectedUsers.includes(user.id) ? 'ring-2 ring-blue-500' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => changeRole(user.id, e.target.value)}
                    disabled={user.id === currentUser?.id || isReadOnly}
                    className={`px-2 py-1 rounded-lg text-xs font-medium ${getRoleBadgeClass(user.role)} border-0 disabled:opacity-50`}
                ...
                {user.fullName && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{user.fullName}</p>
                )}
                ...
                <div className="mt-2 space-y-1">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 truncate">
                    ...
                    <span className="truncate">{user.email || <span className="italic text-gray-400">yo'q</span>}</span>
                  </p>
                  {/* Registratsiyada kiritilgan telefon */}
                  {user.phone && (
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                  <div className="flex gap-4 text-sm">
                    <span className="text-gray-500 dark:text-gray-400">
                      <span className="font-semibold text-gray-900 dark:text-white">{user.totalXP || 0}</span> XP
                    </span>
                    <span className="text-gray-500 dark:text-gray-400">
                      <span className="font-semibold text-gray-900 dark:text-white">{user.completedTests || 0}</span> test
                    </span>
                  </div>
...
function MessageModal({ userIds, users, onClose, onSuccess, token, API }: {
  userIds: string[];
  users: User[];
  onClose: () => void;
  onSuccess: () => void;
  ...
  const [channel, setChannel] = useState<'email' | 'telegram' | 'notification' | 'all'>('notification');
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const sentCount = data.notifSent || data.emailSent || data.telegramSent || data.recipientsCount || userIds.length;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" title="Yopish">
