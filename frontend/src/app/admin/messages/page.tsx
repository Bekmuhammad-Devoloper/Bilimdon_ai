...
export default function AdminMessages() {
  ...
  const [selectedChannels, setSelectedChannels] = useState<string[]>(['notification']);
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      if (!searchTerm.trim()) {
        ...
        usersToSelect = users.filter(u => 
          u.username?.toLowerCase().includes(search) ||
          u.fullName?.toLowerCase().includes(search) ||
          u.email?.toLowerCase().includes(search)
        );
      }
      if (selectedChannels.length === 1 && selectedChannels.includes('telegram')) {
  ...
  const toggleChannel = (ch: string) => {
    if (ch === 'all') {
      setSelectedChannels(selectedChannels.length === 3 ? ['notification'] : ['notification', 'email', 'telegram']);
      return;
    }
    if (selectedChannels.includes(ch)) {
      if (selectedChannels.length > 1) setSelectedChannels(selectedChannels.filter(c => c !== ch));
    } else {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (mediaFile) {
        ...
        console.log('[Upload] Starting upload for file:', mediaFile.name, 'Size:', mediaFile.size, 'Type:', mediaFile.type);
        try {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
        <h3 className="text-lg font-semibold mb-4 dark:text-white">Xabar yozish</h3>

        {/* Kanallar */}
        <div className="mb-6">
          <label className="block text-sm font-medium mb-3 dark:text-gray-300">Kanallar</label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            >
              ...
              <span className="text-sm font-medium dark:text-gray-300">Sayt</span>
              {selectedChannels.includes('notification') && (
            ...
            >
              ...
              <span className="text-sm font-medium dark:text-gray-300">Telegram</span>
              {selectedChannels.includes('telegram') && (
                <div className="absolute -top-2 -right-2 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
                  <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
            ...
            >
              ...
              <span className="text-sm font-medium dark:text-gray-300">Hammasi</span>
              {selectedChannels.length === 3 && (
                <div className="absolute -top-2 -right-2 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center">
                  <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
        ...
        {/* Media */}
        <div className="mb-6">
          <label className="block text-sm font-medium mb-3 dark:text-gray-300">Media</label>
          <div className="flex gap-3 mb-4">
            <button
              onClick={() => { setMediaType('none'); setMediaFile(null); setMediaUrl(''); setMediaPreview(null); }}
          ...
          {mediaType !== 'none' && (
            <div className="space-y-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
              ...
              <input
                type="text"
                value={mediaUrl}
                onChange={(e) => { setMediaUrl(e.target.value); setMediaFile(null); setMediaPreview(e.target.value); }}
        ...
        <button
          onClick={sendMessage}
          disabled={sending || uploading || !message.trim() || selectedUsers.length === 0 || isReadOnly}
          ...
          title={isReadOnly ? "Faqat ko'rish rejimi - xabar yuborishga ruxsat yo'q" : "Xabar yuborish"}
        >
          {sending || uploading ? (
            <>
