...
export default function AdminDesign() {
  ...
  const API = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:2006';
  ...
  const handleDarkVideoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
  ...
  const resetToDefault = async () => {
    if (!confirm('Barcha dizayn sozlamalarini asl holatiga qaytarishni xohlaysizmi?')) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Dizayn sozlamalari</h1>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Video Settings */}
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Video sozlamalari</h3>
            
            <div className="space-y-4">
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-gray-700 dark:text-gray-300">Video takrorlansin (loop)</span>
                <div className="relative">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-14 h-8 rounded-full transition ${videoLoop ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
                    <div className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${videoLoop ? 'translate-x-6' : ''}`} />
                  </div>
                </div>
              </label>

              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-gray-700 dark:text-gray-300">Ovozi o'chirilgan (muted)</span>
                <div className="relative">
                  <input
                    type="checkbox"
                    checked={videoMuted}
                    ...
                    className="sr-only"
                  />
                  <div className={`w-14 h-8 rounded-full transition ${videoMuted ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
                    <div className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full transition-transform ${videoMuted ? 'translate-x-6' : ''}`} />
                  </div>
                </div>
              </label>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Ko'rish</h3>
            <div className="flex gap-2">
              <button
                onClick={() => setPreviewMode('light')}
                className={`px-3 py-1.5 rounded-lg text-sm transition ${previewMode === 'light' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'}`}
              >
                Light
              </button>
              <button
                onClick={() => setPreviewMode('dark')}
                className={`px-3 py-1.5 rounded-lg text-sm transition ${previewMode === 'dark' ? 'bg-gray-700 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'}`}
              >
                Dark
              </button>
            </div>
          ...
          <div className={`relative rounded-lg overflow-hidden aspect-video ${previewMode === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className={`p-6 rounded-xl ${previewMode === 'dark' ? 'bg-gray-800/80 text-white' : 'bg-white/80 text-gray-900'}`}>
          ...
          <p className="mt-4 text-sm text-gray-500 text-center">
            {previewMode === 'light' ? 'Light' : 'Dark'} rejim uchun video ko'rinishi
          </p>
        </div>
      </div>
    </div>
